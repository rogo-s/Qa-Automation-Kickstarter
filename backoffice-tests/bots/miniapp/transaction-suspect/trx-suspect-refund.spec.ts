...
test.describe('BOT MINIAPP - Transaction Suspect trx-suspect-refund @regression', () => {
  test('1. View: trx-suspect-refund tampil @smoke', async ({ page }) => {
    const m = await MiniappMonitoringPage.open(page, 'transaction-ppob' as any);
    await m.page.goto('https://miniapps-dashboard-internal-playground.lentera-app.id/trx-suspect/trx-suspect-refund');
