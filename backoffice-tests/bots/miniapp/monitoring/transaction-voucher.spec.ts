...
/**
 * Monitoring Informasi Transaksi Voucher - BOT MINIAPP
 * Probe 28-08-2026: heading "Informasi Transaksi Voucher", search, Pilih Tanggal, Export, view only
 */
test.describe.configure({ mode: 'serial', timeout: 240000 });

test.describe('BOT MINIAPP - Monitoring Informasi Transaksi Voucher @regression', () => {
  test('1. View: heading & tabel tampil @smoke', async ({ page }) => {
    const m = await MiniappMonitoringPage.open(page, 'transaction-voucher');
  ...
  test('3. Date picker: Pilih Tanggal tampil @smoke', async ({ page }) => {
    ...
    if (await dlg.isVisible().catch(()=>false)) {
      ...
      console.log('[TEMUAN] Date picker dialog tidak muncul transaction-voucher');
    }
  });
