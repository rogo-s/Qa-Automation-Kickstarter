 ...
 * Probe direct goto 28-08-2026: heading "Bank", tambah {"ada" if hasAdd else "tidak ada"}, search "Cari Bank", menu Edit|Edit Fee Bank|Hapus
...
test.describe('BOT MINIAPP - Master Bank @regression', () => {

  test('1. Validasi & search: tabel tampil, validasi Simpan @smoke', async ({ page }) => {
    const m = await MiniappMasterPage.open(page, 'manage-bank');
    ...
    if (await m.hasAddButton()) {
      await m.openAddForm();
      await expect(m.isSaveDisabled()).resolves.toBeTruthy().catch(()=> console.log('[TEMUAN] Simpan enabled saat kosong manage-bank'));
      ...
      console.log('[INFO] manage-bank tidak ada Tambah — view only, skip validasi add');
    }
  });
  ...
  test('3. Edit: search kalau ada → ubah lalu verifikasi @smoke', async ({ page }) => {
    ...
    if (!firstText.trim()) { console.log('[TEMUAN] Tidak ada row untuk edit manage-bank'); return; }
    // ambil kode/name pertama sebagai keyword
    const keyword = firstText.split('\n')[1]?.trim().split(' ')[0] || firstText.trim().split(' ')[0];
    await m.openRowMenu(keyword).catch(()=> console.log('[TEMUAN] Open menu gagal manage-bank'));
      ...
      if (hasUbah>0) {
        ...
        if (await form.count()>0) {
          ...
          if (await input.count()>0) {
            ...
            await m.save().catch(()=> console.log('[TEMUAN] Save edit gagal manage-bank'));
          } else {
            await m.cancelAdd();
          }
        } else {
  ...
  test('4. Delete cleanup: jika ada Tambah, sudah dihapus di test 2; untuk view-only cek Hapus tersedia @smoke', async ({ page }) => {
    ...
    console.log('[INFO] Hapus menu count for manage-bank', hasHapus);
    await m.page.keyboard.press('Escape');
