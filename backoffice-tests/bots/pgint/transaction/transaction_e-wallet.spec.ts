...
test.describe('BOT PGINT - Transaksi E-Wallet @regression', () => {
  test('1. View: Transaksi E-Wallet tampil @smoke', async ({ page }) => {
    const m = await PgintMasterPage.open(page, 'merchant');
    await m.page.goto('https://backoffice-pg-playground.lentera-app.id/transaction/information/transaction_e-wallet');
    await expect(m.page.getByRole('heading', { name: /E-Wallet/i }).first()).toBeVisible({timeout:15000});
