...
/**
 * Master Merchant - BOT PGINT
 * Probe 28-08-2026: heading "Merchant", no Tambah untuk role subandonorogo (view-only)
 * Pola: view/search → validasi Edit → Edit lalu revert → Delete check (tidak hapus real)
 */
test.describe.configure({ mode: 'serial', timeout: 240000 });

test.describe('BOT PGINT - Master Merchant @regression', () => {
  ...
  test('3. Edit: search kalau ada → Ubah lalu verifikasi @smoke', async ({ page }) => {
    ...
    if (!txt.trim()) { console.log('[TEMUAN] Tidak ada row untuk edit merchant'); return; }
    const kw = txt.split('\n')[1]?.trim().split(' ')[0] || txt.trim().split(' ')[0];
    await m.openRowMenu(kw);
    const hasUbah = await m.page.getByRole('menuitem', {name: /Ubah/}).count();
    if (hasUbah===0) { console.log('[TEMUAN] Ubah tidak ada merchant'); await m.page.keyboard.press('Escape'); return; }
    ...
    if (await form.count()>0) {
      ...
      await m.save().catch(()=> console.log('[TEMUAN] Save edit gagal merchant'));
      await m.page.waitForTimeout(1000);
      console.log('[INFO] Edit merchant selesai');
    } else {
      console.log('[TEMUAN] Form tidak muncul setelah Ubah merchant');
    }
  });
