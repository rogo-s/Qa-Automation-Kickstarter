...
/**
 * Master Role - BOT PGINT
 * Probe 28-08-2026: heading "Role", no Tambah untuk role subandonorogo (view-only)
 * Pola: view/search → validasi Edit → Edit lalu revert → Delete check (tidak hapus real)
 */
test.describe.configure({ mode: 'serial', timeout: 240000 });

test.describe('BOT PGINT - Master Role @regression', () => {
  test('1. View: heading & tabel tampil @smoke', async ({ page }) => {
    const m = await PgintMasterPage.open(page, 'role');
  ...
  test('3. Edit: search kalau ada → Ubah lalu verifikasi @smoke', async ({ page }) => {
    ...
    if (!txt.trim()) { console.log('[TEMUAN] Tidak ada row untuk edit role'); return; }
    const kw = txt.split('\n')[1]?.trim().split(' ')[0] || txt.trim().split(' ')[0];
    await m.openRowMenu(kw);
    const hasUbah = await m.page.getByRole('menuitem', {name: /Ubah/}).count();
    if (hasUbah===0) { console.log('[TEMUAN] Ubah tidak ada role'); await m.page.keyboard.press('Escape'); return; }
    ...
    if (await form.count()>0) {
      ...
      console.log('[INFO] Edit role selesai');
    } else {
      console.log('[TEMUAN] Form tidak muncul setelah Ubah role');
    }
  });
  test('4. Delete check: Hapus menu ada (tidak hapus data real) @smoke', async ({ page }) => {
    ...
    console.log('[INFO] Hapus tersedia untuk role, tidak hapus data real biar tidak sampah');
  });
