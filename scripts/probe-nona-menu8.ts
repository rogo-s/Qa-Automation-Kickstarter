...
(async () => {
  ...
  console.log('title only disabled:', await save());
  // isi semua tanpa switch
  ...
  await webview.waitForTimeout(500);
  console.log('all fields no switch disabled:', await save());
  await form.locator('[role="switch"]').click({ force: true });
  await webview.waitForTimeout(500);
  console.log('with switch checked disabled:', await save());
  await browser.close();
})();
