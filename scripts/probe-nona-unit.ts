...
(async () => {
  ...
  const buttons = await webview.locator('main button').allTextContents();
  console.log('buttons:', JSON.stringify(buttons.map(t => t.trim()).filter(Boolean)));
  ...
  console.log('tbody empty?:', JSON.stringify((await webview.locator('main tbody').textContent())?.trim().slice(0, 60)));
  await browser.close();
})();
